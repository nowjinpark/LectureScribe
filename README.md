...
빌드 결과는 로컬 실행용 임시 서명이 적용된 앱입니다. Developer ID 서명과 공증을 거친 배포본은 아닙니다. SDK 선택은 [개발 가이드](docs/DEVELOPMENT.md)를 참고하세요.

1. **작업 폴더 → 변경**에서 강의를 모아둘 위치를 선택합니다.
2. **새 강의 녹음**에서 제목과 언어를 정합니다. Zoom만 녹음하려면 Zoom을 실행하고 **실행 중인 앱 불러오기**에서 선택합니다.
3. 첫 녹음 시 macOS의 **개인정보 보호 및 보안 → 화면 및 시스템 오디오 녹음** 권한을 허용합니다. 영상과 마이크 입력은 저장하지 않습니다.
